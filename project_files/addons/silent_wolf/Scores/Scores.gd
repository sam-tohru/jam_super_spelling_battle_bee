...
func get_scores(maximum: int=10, ldboard_name: String="main", period_offset: int=0) -> Node:
	...
	latest_max = maximum
	var request_url = "https://api.silentwolf.com/get_scores/" + str(SilentWolf.config.game_id) + "?version=" + str(SilentWolf.config.game_version) + "&max=" + str(maximum)  + "&ldboard_name=" + str(ldboard_name) + "&period_offset=" + str(period_offset)
...
func get_scores_by_player(player_name: String, maximum: int=10, ldboard_name: String="main", period_offset: int=0) -> Node:
	if player_name == null:
		...
		# resetting the latest_number value in case the first requests times out, we need to request the same amount of top scores in the retry
		latest_max = maximum
		var request_url = "https://api.silentwolf.com/get_scores_by_player/" + str(SilentWolf.config.game_id) + "?version=" + str(SilentWolf.config.game_version) + "&max=" + str(maximum)  + "&ldboard_name=" + str(ldboard_name.uri_encode()) + "&player_name=" + str(player_name.uri_encode()) + "&period_offset=" + str(period_offset)
...
func _on_GetScoresByPlayer_request_completed(result, response_code, headers, body) -> void:
	...
	if status_check:
		...
		if json_body.success:
			...
			player_scores = json_body.top_scores
...
func get_top_score_by_player(player_name: String, maximum: int=10, ldboard_name: String="main", period_offset: int=0) -> Node:
	if player_name == null:
		...
		# resetting the latest_number value in case the first requests times out, we need to request the same amount of top scores in the retry
		latest_max = maximum
		var request_url = "https://api.silentwolf.com/get_top_score_by_player/" + str(SilentWolf.config.game_id) + "?version=" + str(SilentWolf.config.game_version) + "&max=" + str(maximum)  + "&ldboard_name=" + str(ldboard_name.uri_encode()) + "&player_name=" + str(player_name.uri_encode()) + "&period_offset=" + str(period_offset)
...
func _on_GetTopScoreByPlayer_request_completed(result, response_code, headers, body) -> void:
	...
	if status_check:
		...
		var sw_result: Dictionary = SilentWolf.build_result(json_body)
		if json_body.success:
			SWLogger.info("SilentWolf get top score by player success, found top score? " + str(!json_body.top_score.empty()))
			player_top_score = json_body.top_score
...
func get_scores_around(score, scores_to_fetch=3, ldboard_name: String="main") -> Node:
	...
	# resetting the latest_number value in case the first requests times out, we need to request the same amount of top scores in the retry
	#latest_max = maximum
	var request_url = "https://api.silentwolf.com/get_scores_around/" + str(SilentWolf.config.game_id) + "?version=" + str(SilentWolf.config.game_version) + "&scores_to_fetch=" + str(scores_to_fetch)  + "&ldboard_name=" + str(ldboard_name) + "&score_id=" + str(score_id) + "&score_value=" + str(score_value)
...
func _on_ScoresAround_request_completed(result, response_code, headers, body) -> void:
	...
	if status_check:
		...
		if json_body.success:
			...
			sw_result["scores_above"] = json_body.scores_above
			sw_result["scores_below"] = json_body.scores_below
...
func wipe_leaderboard(ldboard_name: String='main') -> Node:
	...
	var payload = { "game_id": SilentWolf.config.game_id, "game_version": SilentWolf.config.game_version, "ldboard_name": ldboard_name }
